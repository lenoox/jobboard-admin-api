package com.lenoox.jobboard.auth;
...
public class JwtConverter implements Converter<Jwt, JwtDto> {
    @Override
    public JwtDto convert(Jwt source) {
        List<GrantedAuthority> grantedAuthorities = extractAuthorities(source);
        var customJwt = new JwtDto(source, grantedAuthorities);
